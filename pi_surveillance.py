...
def getDropBoxClient(conf):
    ...
    if conf["use_dropbox"]:
        ...
        logger.info("[SUCCESS] dropbox account linked")
        return client
    else:
        return None


def captureAndProcess(conf, client, gmail_password):
    ...
    for f in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        ts = timestamp.strftime("%Y%h%d-%H%M%S")
            ...
            if (timestamp - lastUploaded).seconds >= conf["min_upload_seconds"]:
                ...
                if motionCounter >= conf["min_motion_frames"]:
                    ...
                    if conf["use_dropbox"]:
                        ...
                        logger.info("Sending email with {} as attachment".format((ts)))
...
if __name__ == '__main__':
    ...
    client = getDropBoxClient(conf)    
    captureAndProcess(conf, client, gmail_password)
